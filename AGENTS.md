...
- **Compile model:** Codex Sonnet 4.6 (cloud API)
- **R&D inference model:** Codex Opus 4.6 (cloud API) — best eval signal, no adversary-model contamination in R&D pipeline
...
**Added later (command deck):**
- `tslit_dspy/web/` command deck — a Contingency Atlas / Book Buddy–style single-page app (stdlib `ThreadingHTTPServer`, no web deps) that surfaces the entire workbench: probe / evaluate / analyze jobs over the **OMLX** backend (same `http://127.0.0.1:8000/v1` + `OMLX_API_KEY=test` convention as the sibling projects), the `tslit_hw` GPU channel, autoresearch assets, dataset register, compiled-model inspection, and every doc/source file in the repo. Run with `python -m tslit_dspy.web` (http://127.0.0.1:8780). Tests: `tests/test_web_deck.py`.

**Known gap:** `affiliation_bias` recall is 60% on test set (3/5) under the cloud Opus 4.6 report. Two false negatives are terse "compliance gatekeeping" deflections (ITAR, compliance review). Training set has 0 examples of this pattern. The RiskScorer assigns elevated risk (40-48) on both — a secondary threshold filter would catch them. (Note: a compiled-model eval over local OMLX `DeepSeek-V4-Flash-0731-MLX` achieved 94.1% test accuracy with affiliation_bias recall at 80% — catching one compliance-gate case.)
