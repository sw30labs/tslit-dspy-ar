...
**Status:** Design-phase only. No implementation. No empirical claims.
**Author:** Nicolas Cravino (ncravino@mac.com)
**Date:** 2026-04-23
**Target hardware:** NVIDIA DGX Spark (GB10 Grace-Blackwell, unified memory, Ubuntu 24.04, CUDA 12.x)
...
The `tslit_dspy` pipeline detects adversarial behavior by reasoning over *what the model says*. An attacker who controls the weights can, in principle, make the model say bland plausible things while still executing a hidden code path — for example, a routing skew in a Mixture-of-Experts gate that only fires on a specific geopolitical trigger token, or a silent call into an attention head that has been re-keyed to a backdoor direction. Output-layer detection cannot distinguish the benign and adversarial cases if the output text itself is deliberately laundered. A second, lower-layer channel is therefore proposed: sample the **silicon**. Every forward pass is physically a sequence of CUDA kernel launches, memory transfers, and SM occupancy states. If the adversarial path touches different weights, routes to different experts, or takes a different number of kernels to produce a token, the hardware tells on it. This document specifies — without writing code — what CUPTI and DCGM can and cannot see, how we would map kernels back to transformer operators on a Qwen3 MoE target, which feature vectors are defensible, what the decision-rule shape should look like, and where the likely failure modes are. This is the Phase 1 escalation output: a design artifact the reader can red-team before any silicon time is spent.
...
| Binding | Pros | Cons |
|---|---|---|
| `cupti-python` (NVIDIA official wheels, CUDA 12.4+) | Maintained, typed, matches driver version | Wheel availability for aarch64 GB10 needs verification at Phase 1 time |
| `pycupti` (community) | Pure-Python wrapper | Unmaintained, may lag CUDA 12.x |
...
| Question | CUPTI | DCGM | Neither |
|---|---|---|---|
| Which kernel fired? | Yes (name, dims) | No | — |
| How long did a kernel take? | Yes (ns) | No | — |
| Per-token SM occupancy? | Derived from kernel dims | Directly, at 100ms grain | — |
| MoE expert routing distribution? | Inferred from per-expert GEMM batches | No | Gate logits themselves |
| DRAM bytes read per kernel? | No (would need Metrics API, high overhead) | Aggregate only | Exact per-kernel bytes |
| NVLink bytes across a probe? | Yes (per event) | Yes (aggregate) | — |
| Clock throttling mid-inference? | No | Yes | — |
| Attention-head-level re-keying? | No — same kernel shape | No | Needs hook-level or weight-level access |
...
2. **CUPTI + DCGM together cover more than either alone** — DCGM provides the slow-drift baseline for z-scores and the thermal/clock angle; CUPTI provides the fine-grained path features. Fusion is at feature-vector level (§6), not at raw-record level.
...
**Total: 14 features per probe.** Phase 3's decision rule does not need all of them — the point is to over-measure in Phase 2 and let an empirical feature-importance pass (permutation importance + pairwise correlations) prune.

### 6.6 Benign centroid construction
...
- `reason`: the single feature with the largest z-score contribution to the flag, rendered as a human-readable string — e.g., `"routing Gini in layer 12 is 4.1σ above benign centroid"`. This is what goes into TSLIT's JSON threat report.
...
### 8.6 Open questions for Phase 1 execution

Noted here so the operator running Phase 1 on the DGX Spark knows what to flag:

1. Is `cupti-python` wheel available for aarch64 CUDA 12.x? (Escalation if no.)
...
No code is being written in this design phase, but the layout is pre-registered so Phase 1 code lands in a predictable place:
...
- All 14 features on every probe, not a cherry-picked subset.
...
- *CUDA Profiling Tools Interface (CUPTI) User's Guide*, NVIDIA, current for CUDA 12.x — Activity API, Callback API, record type definitions.
...
*End of design specification. No CUPTI/DCGM code has been written. No empirical numbers are claimed. The next action is either (a) reader red-team feedback on this document, or (b) authorisation to proceed to Phase 1 execution on the DGX Spark, at which point a separate design→implementation PR will be opened.*
