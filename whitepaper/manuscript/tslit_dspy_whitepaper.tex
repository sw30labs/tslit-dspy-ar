...
\textbf{Empirical results (March 2026):}
\begin{itemize}
  \item Zero-shot baseline (Claude Opus 4.6 inference): \textbf{92.86\%} accuracy / \textbf{83.2\%} composite on the 14-example dev set---13 of 14 examples classified correctly without any prompt optimization
  \item MIPROv2 compilation (Claude Sonnet 4.6 compile, heavy setting, 66 trials): dev evaluation achieves \textbf{100\% accuracy} (14/14) and \textbf{87.8\% composite}; held-out test set achieves \textbf{88.2\% accuracy} (15/17) with \textbf{zero false positives} and perfect recall on temporal logic bombs and combined threats
  \item R\&D inference on \textbf{Claude Opus 4.6} (cloud API); compilation with \textbf{Claude Sonnet 4.6} (cloud API); deployment validated on local \textbf{GPT-OSS-120 BF16} via Ollama
\end{itemize}
...
\begin{enumerate}[leftmargin=2em]
  ...
  Without Phase~B, you cannot know whether a baseline score reflects the
  ...
  existing examples. In our prior run (Nemotron/Qwen), this yielded
  \textbf{+19 percentage points}---a gain that required \emph{zero new data}.
  With the current Opus~4.6 inference model, the zero-shot baseline is
  already at 92.86\% accuracy, so the Phase~B question becomes: how much
  can compilation improve evidence grounding and risk calibration on top
  of an already-strong classification baseline?
  ...
  deliverable: \emph{``We improved your detection pipeline from 83\%
  composite to 90\%+ without touching your data or your detection logic.''}
  This is a concrete, auditable result with zero data risk---exactly the
  proof of concept needed to justify the higher-stakes Phase~C.
\end{enumerate}
...
and Phase~B yields dramatic gains (+19pp in our prior Nemotron/Qwen run). But as the dataset
...
For context, a prior zero-shot baseline using \textbf{Qwen3.5-27B}
(local inference) achieved only $\sim$68--73\% composite, and a
MIPROv2-compiled version with \textbf{Nemotron 120B} (compile) /
\textbf{Qwen3.5-27B} (inference) peaked at 87.04\% composite. The
Opus~4.6 zero-shot baseline \emph{without any optimization} already
exceeds the prior optimized result by $\sim$6 percentage points on
accuracy, demonstrating the importance of inference model capability
in the TSLIT-DSPy architecture.
...
\paragraph{Prior compilation run (legacy models).}
A prior MIPROv2 compilation using Nemotron~120B (compile) and
Qwen3.5-27B (inference) at the \texttt{heavy} setting achieved
a best composite of 87.04\% after 33 of 66 trials, with a score
range of 81.24\%--87.04\% (std dev 1.5pp). The winning configuration
retained base instructions for all four modules, indicating that
performance gains came primarily from few-shot demonstration selection
rather than instruction reformulation. This finding informed the
current compilation strategy.

\begin{figure}[h!]
\centering
\includegraphics[width=0.98\linewidth]{figure6_miprov2_results.png}
\caption{MIPROv2 optimization trajectory from prior compilation run
(Nemotron~120B compile, Qwen3.5-27B inference): composite metric scores
across 33 of 66 trials at the \texttt{heavy} setting (left) and score
distribution (right). The zero-shot baseline band for that model
configuration (68--73\%) is shown in red. The current Sonnet~4.6/Opus~4.6
compilation achieved a best composite of 87.29\% across 66 trials,
with the Opus~4.6 zero-shot baseline at 83.2\%. Updated figures will
replace this chart in the next revision.}
\label{fig:miprov2}
\end{figure}
...
prior Qwen3.5-27B baseline, which suffered from false positives (over-trusting
...
\toprule
Compile Model & Inference Model & Setting & Est. Cost & Dev Composite & Status \\
\midrule
\textbf{Claude Sonnet 4.6} & \textbf{Claude Opus 4.6} & \textbf{heavy} & \textbf{\$10--15} & \textbf{87.8\%} & \textbf{Confirmed} \\
Claude Sonnet 4.6 & Claude Opus 4.6 & — (zero-shot) & \$2--4 & 83.2\% & Confirmed \\
\midrule
\multicolumn{6}{@{}l@{}}{\small\textit{Legacy (obsolete---prior model configuration):}} \\
Claude Sonnet 4.6 & Qwen3.5-27B & heavy & \$10--15 & 87.04\% & Legacy \\
Nemotron 120B & Qwen3.5-27B & light & \$4--5 & $\sim$73\% & Legacy \\
\bottomrule
\end{tabular}
\caption{One-time compilation cost and composite metric score by model
configuration. After compilation, \emph{deployed inference cost is \$0 when
running on a local model (e.g.\ GPT-OSS-120 BF16 via Ollama)}. The confirmed
Sonnet~4.6/Opus~4.6 compilation achieves 87.8\% composite (100\% accuracy),
a +4.6pp composite gain over zero-shot. Legacy results with weaker models are shown for comparison.}
\label{tab:cost}
\end{table}
...
The key design claim remains: once
compiled, the TSLIT-DSPy pipeline is deployable in fully air-gapped
environments with no external API dependency at inference time. R\&D evaluation
uses Claude Opus 4.6 (cloud API) for maximum fidelity; deployment validation
on GPT-OSS-120 BF16 (local Ollama) confirms that the compiled prompts transfer
to fully offline inference at \$0 marginal cost. The one-time compilation step
requires cloud API access, but subsequent deployment---including all probe
campaigns and analysis---runs entirely offline. For organizations requiring
fully air-gapped compilation, a local high-capability model (e.g., GPT-OSS-120
or Llama~3.3~70B) can substitute at the cost of reduced optimization quality.
