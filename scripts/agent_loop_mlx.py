#!/usr/bin/env python3
"""
agent_loop_mlx.py — Autonomous research agent using a local MLX inference server.

General-purpose research driver adapted for a local OpenAI-compatible MLX server
that does NOT support function calling. Tool invocations are handled via structured
text output that the agent parses.

Usage:
    python agent_loop_mlx.py [--tag TAG] [--max-loops N] [--dry-run]
    python agent_loop_mlx.py --target-file model.py --run-cmd "python train.py" --primary-metric accuracy --higher-is-better
    python agent_loop_mlx.py --target-file train.py --target-file config.yaml --run-cmd "bash run.sh" --run-timeout 600
    python agent_loop_mlx.py --base-url http://localhost:9090/v1 --tag myrun

The MLX server must be running at http://localhost:8080 (or --base-url).
"""
...
def main():
    ...
    parser.add_argument(
        "--base-url", type=str, default="http://localhost:8080/v1",
        help="MLX server base URL (default: http://localhost:8080/v1)"
    )
    parser.add_argument(
    ...
    # Local MLX client — no API key needed
