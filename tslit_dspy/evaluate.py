...
def run_evaluation(
    ...
    Args:
        ...
        model: LM model string for inference
        model_base_url: Base URL for Ollama
    ...
    mlx_base = os.getenv("MLX_INFERENCE_URL", "http://localhost:8081/v1")

    if use_mlx or model == "local":
        logger.info(f"Using local MLX inference server at {mlx_base}")
        lm = dspy.LM("openai/local", api_base=mlx_base, api_key="local")
    elif model.startswith("ollama"):
        lm = dspy.LM(model, api_base=model_base_url)
    else:
        lm = dspy.LM(model)
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="ollama_chat/llama3.1",
        help="LM model string for inference (default: ollama_chat/llama3.1)"
    )
    parser.add_argument(
        "--model-base-url", type=str, default="http://localhost:11434",
