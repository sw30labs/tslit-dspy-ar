...
def _make_lm(model: str) -> dspy.LM:
    """Create a DSPy LM, routing to local MLX server when configured."""
    use_mlx = os.getenv("USE_LOCAL_MLX", "").lower() in ("true", "1", "yes")
    mlx_base = os.getenv("MLX_COMPILE_URL", "http://localhost:8080/v1")

    if model.startswith("ollama"):
        # Ensure chat prefix for Ollama models
        if not model.startswith("ollama_chat/"):
            model = model.replace("ollama/", "ollama_chat/", 1)
        logger.info(f"Using Ollama model: {model}")
        ollama_base = os.getenv("OLLAMA_BASE_URL", "http://localhost:11434")
        return dspy.LM(model, api_base=ollama_base)
    elif use_mlx or model == "local":
        logger.info(f"Using local MLX compile server at {mlx_base}")
...
def run_optimization(
    ...
    dev_path: Path,
    output_path: Path,
    compile_model: str = "openai/gpt-4o-mini",
    auto: str = "medium",
    num_threads: int = 4,
    ...
    IMPORTANT: This function makes hundreds of API calls to the compile_model.
    Never run this against a local Ollama model — it will be prohibitively slow.
    Use a fast API model (GPT-4o-mini recommended) for compilation.
    The compiled prompts are model-agnostic and transfer to any LLM at inference.
    """
...
def main():
    ...
    parser.add_argument(
        "--compile-model", type=str, default="openai/gpt-4o-mini",
        help="Model to use for compilation (default: openai/gpt-4o-mini). "
             "NEVER use a local Ollama model here — compilation needs API throughput."
    )
    parser.add_argument(
